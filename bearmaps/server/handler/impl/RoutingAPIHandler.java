...
/**
 * Handles requests from the web browser for routes between locations. The route
 * will be returned as image data, as well as (optionally) driving directions. *
 * Created by rahul
 */
public class RoutingAPIHandler extends APIRouteHandler<Map<String, Double>, Map<String, Object>> {

    /**
     * Each route request to the server will have the following parameters as keys
     * in the params map.<br>
     ...
     * street directions between the given points. THis method has been completed
     * for you.
     *
     * The route to draw on the map should be added to the end of
     * Constants.ROUTE_LIST. This is a LinkedList of longs, where each long
     * corresponds to one point on the map.
     *
     * Street directions can also be provided in the form of text as a return value
     * to this function.
     *
     * @param requestParams Map of the HTTP GET request's query parameters - the
     *                      starting lat/long and the destination lat/lon.
     *
     * @param response      : Not used by this function. You may ignore.
     * @return A map of results for the front end as specified: <br>
     *         "routing_success" : Boolean, whether the route list should be drawn
     *         (i.e. if not empty).
     *
     *         "directions_success" : Boolean, whether the query generated text
     *         directions (i.e. if length of directions is > 0). "directions" :
     *         String. The text directions you want to display, in HTML format.
     */
    @Override
    protected Map<String, Object> processRequest(Map<String, Double> requestParams, Response response) {
        List<Long> route = Router.shortestPath(SEMANTIC_STREET_GRAPH, requestParams.get("start_lon"),
                requestParams.get("start_lat"), requestParams.get("end_lon"), requestParams.get("end_lat"));
